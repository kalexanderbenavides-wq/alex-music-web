<script>
  // Añadido 'label' para evitar el crasheo de renderizado
...
<div class="relative flex flex-col items-center group">
  <div class="relative flex justify-center w-12" style="height: {height}px;">
    ...
    <div class="absolute w-1.5 h-full bg-[#050505] rounded-full shadow-[inset_0_2px_5px_rgba(0,0,0,1)] border border-[#1a1a1a] overflow-hidden">
      ...
      <div 
        class="absolute bottom-0 left-0 w-full transition-all duration-100 ease-out" 
        style="height: {bottomPos}%; background: linear-gradient(to top, #34c759 0%, #ffcc00 50%, #ff3b30 100%); background-size: 100% {height}px; background-position: bottom;"
      ></div>
    ...
    <div class="absolute w-10 h-5 bg-gradient-to-b from-[#2a2a2a] to-[#111] rounded-sm border border-[#333] shadow-[0_5px_10px_rgba(0,0,0,0.8)] pointer-events-none z-10 flex items-center justify-center transition-all duration-100 ease-out" style="bottom: calc({bottomPos}% - 10px);">
      <div class="w-full h-[2px] shadow-[0_0_8px_var(--glow)] transition-colors duration-300" style="background-color: {ledColor}; --glow: {ledColor};"></div>
    </div>
  ...
  <div class="mt-4 flex flex-col items-center">
    <span class="text-[0.70rem] font-mono font-bold transition-colors duration-300 drop-shadow-[0_0_5px_var(--glow)]" style="color: {ledColor}; --glow: {ledColor};">
      {Math.round(value)}
    </span>
    {#if label}
